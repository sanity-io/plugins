...
import {getPaths} from '../sanity/manifest'
import type {ManifestOptions} from '../sanity/manifest'
...
export function addScript(cmd: string, existing: string | undefined) {
...
export async function addBuildScripts(manifest: PackageJson, options: InjectOptions) {
  ...
  return addPackageJsonScripts(manifest, options, (scripts) => {
    ...
    scripts['prepublishOnly'] = addScript(
      expectedScripts['prepublishOnly'],
