...
import type {Injectable, InjectOptions} from '../actions/inject'
import {writeAssets} from '../actions/inject'
...
function files(): Injectable[] {
  ...
  return base.map((fromTo) => {
    if (fromTo.type === 'copy') {
      const fromArr = typeof fromTo.from === 'string' ? [fromTo.from] : fromTo.from
      return Object.assign({}, fromTo, {
        from: ['ui-workshop', ...fromArr],
      })
    }
