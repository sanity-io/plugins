...
export async function isEmptyish(dirPath: string) {
  const ignoredFiles = new Set(['.git', '.gitignore', 'license', 'readme.md'])
  const allFiles = await readdir(dirPath).catch(() => [])
  const files = allFiles.filter((file) => !ignoredFiles.has(file.toLowerCase()))
...
export function parseJson5<T>(content: string, errorKey: string): T {
  try {
    ...
    throw new Error(`Error parsing "${errorKey}": ${err.message}`, {cause: err})
  }
}
