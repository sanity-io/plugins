...
export function validateNodeEngine(packageJson: PackageJson): string[] {
...
export async function validatePluginSanityJson({
  ...
  const expectedPart = expectedDefaults.parts[0]!
  const correctImplements = firstPart?.implements === expectedPart.implements
  const pathExists =
  ...
  if (!isValid) {
    const errors = [
      ...
      !correctImplements
        ? `The part should implement ${expectedPart.implements}, but did not.`
        : null,
...
export function validatePackageName(packageJson: PackageJson): string[] {
