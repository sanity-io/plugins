...
async function writeStaticAssets(options: InjectOptions) {
  ...
  const files: Injectable[] = (
    [
      ...
      {type: 'copy' as const, from: 'editorconfig', to: '.editorconfig'},
      {type: 'copy' as const, from: 'sanity.json', to: 'sanity.json'},
      {type: 'copy' as const, from: 'v2-incompatible.js.template', to: 'v2-incompatible.js'},
      ...
      flags.prettier && {type: 'copy' as const, from: 'prettierrc.json', to: '.prettierrc'},
    ] as (Injectable | false | undefined)[]
