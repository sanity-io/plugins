...
const allowedPartProps = new Set(['name', 'implements', 'path', 'description'])
const disallowedPluginProps = new Set(['api', 'project', 'plugins', 'env'])
...
export async function readManifest(options: ManifestOptions) {
  ...
  try {
    ...
    throw new Error(`Failed to read "${manifestPath}": ${err.message}`, {cause: err})
  }
  ...
  try {
    ...
    throw new Error(`Error parsing "${manifestPath}": ${err.message}`, {cause: err})
  }
...
export async function validatePluginManifest(
  ...
  const disallowed = Object.keys(manifest)
    .filter((key) => disallowedPluginProps.has(key))
    .map((key) => `"${key}"`)
...
export async function validatePaths(manifest: SanityV2Manifest, options: {basePath: string}) {
  ...
  try {
    ...
    if (err.code === 'ENOENT') {
      throw new Error(
        `sanity.json references "source" path which does not exist: "${sourcePath}"`,
        {cause: err},
      )
    }
  }
...
function validateAllowedPartKeys(part: Record<string, any>, index: number) {
  const disallowed = Object.keys(part)
    .filter((key) => !allowedPartProps.has(key))
    .map((key) => `"${key}"`)
