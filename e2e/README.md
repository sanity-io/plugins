...
When tests fail, the PR comment includes a **Share with an AI agent** blockquote pointing at that URL — paste it into an agent chat to hand over everything needed to debug the run in a single request. The same digest is appended to the workflow run's job summary, and shipped in the `playwright-report-<run_id>` GitHub artifact.
...
On pull requests, CI posts an **E2E Tests** status comment with pass/fail/flaky/skipped counts, a hosted HTML report URL, dataset names, and a link to the workflow run. On failure it also links the agent-friendly `agent-report.md` (see _Agent-friendly failure report_).
