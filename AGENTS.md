...
2. **Add an `author:` directive to the changeset summary.** This is the important one. Changelogs are generated by `@changesets/changelog-github`, which credits the **PR author** whenever the changeset's commit is associated with a PR. Because a ported PR is opened by someone else (often the `🤖 bot`), the commit author alone is **not** enough — without the directive the changelog would thank the wrong person. An `author:` (or `user:`) line on its own line overrides this and is stripped from the rendered text:
   ...
   author: matthewwyndham

   Add quick "Row" and "Column" buttons to the table input toolbar
   ```

   This produces a changelog line crediting the original author:

   > Thanks [@matthewwyndham](https://github.com/matthewwyndham)! - Add quick "Row" and "Column" buttons to the table input toolbar

   Use the original author's **GitHub username** (the leading `@` is optional). Add multiple `author:` lines to credit more than one person.
