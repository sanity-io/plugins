...
- Only monorepo-required plugin config files are maintained (`package.json`, `tsdown.config.ts` — or `package.config.ts` for the few plugins still built with `@sanity/pkg-utils` — `tsconfig.json`, `vitest.config.ts`)
