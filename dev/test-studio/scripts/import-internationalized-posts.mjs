...
const useOldFormat = args.has('--old-format') || process.env.IMPORT_OLD_FORMAT === 'true'
...
function asInternationalizedPost(post) {
  const title = Object.entries(post.translations).map(([language, localized]) => {
    if (useOldFormat) {
      return {
        _key: language,
  ...
  const description = Object.entries(post.translations).map(([language, localized]) => {
    if (useOldFormat) {
      return {
        _key: language,
...
async function run() {
  ...
  console.warn(`Format: ${useOldFormat ? 'old (_key language)' : 'new (language field)'}`)
