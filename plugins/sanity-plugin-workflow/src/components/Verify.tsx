...
// It will only render something if there is invalid data
...
export default function Verify(props: VerifyProps) {
  ...
  const correctDocuments = useCallback(
    async (ids: string[]) => {
      toast.push({
        title: 'Correcting...',
        status: 'info',
  ...
  const removeUsersFromDocuments = useCallback(
    async (ids: string[]) => {
      toast.push({
  ...
  const addOrderToDocuments = useCallback(
    async (ids: string[]) => {
      toast.push({
  ...
  const resetOrderOfAllDocuments = useCallback(
    async (ids: string[]) => {
      toast.push({
        title: 'Adding ordering...',
        status: 'info',
  ...
  const orphanedMetadataDocumentIds = useMemo(() => {
    return data.length ? data.filter((doc) => !doc?._id).map((doc) => doc._metadata.documentId) : []
  }, [data])

  const handleOrphans = useCallback(async () => {
    toast.push({
