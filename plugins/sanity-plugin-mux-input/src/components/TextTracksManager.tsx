...
export default function TextTracksManager({
  ...
  const derivedAutogeneratedTrackIds = useMemo(() => {
    ...
    autogeneratedTrackIds.forEach((id) => newAutogeneratedIds.add(id))
    return newAutogeneratedIds
  }, [activeTracks, addedTracks, autogeneratedTrackIds])
  ...
  const visibleTracks = allTracks
    ...
    .sort((a, b) => {
      ...
      const aIsAutogenerated =
        (a.id && a.id.startsWith('generating-')) || (a.id && derivedAutogeneratedTrackIds.has(a.id))
      const bIsAutogenerated =
        (b.id && b.id.startsWith('generating-')) || (b.id && derivedAutogeneratedTrackIds.has(b.id))
  ...
  const getTrackSourceLabel = (track: MuxTextTrack) => {
    ...
    if (track.id && derivedAutogeneratedTrackIds.has(track.id)) {
      return 'Auto-generated'
    }
    if (
      track.text_source === 'generated_live_final' ||
