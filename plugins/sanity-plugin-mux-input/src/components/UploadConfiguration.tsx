...
function getValidationError({
  ...
  if (
    ...
    videoAssetMetadata.size > maxFileSize
  ) {
    return `File size (${formatBytes(videoAssetMetadata.size)}) exceeds maximum allowed size of ${formatBytes(maxFileSize)}`
  }

  if (
    videoAssetMetadata?.duration &&
    maxDurationSeconds &&
    videoAssetMetadata.duration > maxDurationSeconds
  ) {
    return `Video duration (${formatSeconds(videoAssetMetadata.duration)}) exceeds maximum allowed duration of ${formatSeconds(maxDurationSeconds)}`
  }

  if (config.drm_policy && videoAssetMetadata?.isAudioOnly) {
    return 'Audio-only asset cannot be DRM protected'
  }
...
export default function UploadConfiguration({
  ...
  const [autoTextTracks] = useState<NonNullable<UploadConfig['text_tracks']>>(() =>
  ...
  const {videoAssetMetadata: rawVideoAssetMetadata, isLoadingMetadata} =
    useMediaMetadata(stagedUpload)
  const videoAssetMetadata = fileSize
    ? {...rawVideoAssetMetadata, size: fileSize}
    : rawVideoAssetMetadata
  ...
  }, [config, secrets, skipConfig, startUpload, videoAssetMetadata?.aspectRatio])
