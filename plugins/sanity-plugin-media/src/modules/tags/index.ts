...
const tagsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(DIALOG_ACTIONS.showTagEdit, (state, action) => {
        ...
        const tagItem = state.byIds[tagId]
        if (tagItem) {
          delete tagItem.error
        }
      })
      .addMatcher(
        isAnyOf(
        ...
        (state, action) => {
          ...
          const tagItem = state.byIds[tag._id]
          if (tagItem) {
            tagItem.updating = false
          }
        },
      )
      .addMatcher(
  ...
  reducers: {
    ...
    deleteError(state, action: PayloadAction<{error: HttpError; tag: Tag}>) {
      ...
      const tagId = tag?._id
      const tagItem = tagId ? state.byIds[tagId] : undefined
      if (tagItem) {
        tagItem.error = error
        tagItem.updating = false
      }
    ...
    deleteRequest(state, action: PayloadAction<{tag: Tag}>) {
      ...
      const deleteTagItem = tagId ? state.byIds[tagId] : undefined
      if (deleteTagItem) {
        deleteTagItem.picked = false
        deleteTagItem.updating = true
      }

      Object.keys(state.byIds).forEach((key) => {
        const item = state.byIds[key]
        if (item) {
          delete item.error
        }
      })
    ...
    updateComplete(state, action: PayloadAction<{closeDialogId?: string; tag: Tag}>) {
      const {tag} = action.payload
      const updateItem = state.byIds[tag._id]
      if (updateItem) {
        updateItem.tag = tag
    ...
    updateError(state, action: PayloadAction<{tag: Tag; error: HttpError}>) {
      const {error, tag} = action.payload
      const tagId = tag?._id
      const errorItem = tagId ? state.byIds[tagId] : undefined
      if (errorItem) {
        errorItem.error = error
        errorItem.updating = false
      }
    },
    updateRequest(
      state,
      ...
      const {tag} = action.payload
      const reqItem = state.byIds[tag._id]
      if (reqItem) {
        reqItem.updating = true
      }
    },
  },
})
  ...
  action$.pipe(
    ...
    mergeMap(([action, state]) => {
      ...
      return of(action).pipe(
        ...
        // oxlint-disable-next-line no-unsafe-type-assertion -- result from Sanity client create is typed as Tag
  ...
  action$.pipe(
    ...
    mergeMap(([action, state]) => {
      ...
      return of(action).pipe(
        ...
        // oxlint-disable-next-line no-unsafe-type-assertion -- Sanity client commit returns Tag
        mergeMap(
          () =>
            from(
              client
...
export const selectTags: Selector<RootReducerState, TagItem[]> = createSelector(
  ...
  (byIds, allIds) => allIds.map((id) => byIds[id]).filter((item): item is TagItem => !!item),
)
