...
import {
  buildExcludeMediaLibraryFragment,
  buildExcludeTagsFragment,
} from '../../utils/constructFilter'
    ...
    switchMap(([action, state]) =>
      of(action).pipe(
        ...
        mergeMap(() => {
          ...
          const excludeMediaLibraryFragment = buildExcludeMediaLibraryFragment(
            state.assets.showMediaLibraryAssets,
          )
          const excludeAssetsClause = [excludeTagsFragment, excludeMediaLibraryFragment]
            .filter(Boolean)
...
export const foldersRenameEpic: MyEpic = (action$, state$, {client}) =>
  action$.pipe(
    ...
    mergeMap(([action, state]) => {
      ...
      const collision = siblingIds.some(
        (id) =>
          id !== folderId && state.folders.byId[id]?.name.toLowerCase() === name.toLowerCase(),
      )
      if (collision) {
        return of(
          foldersSlice.actions.renameError({
