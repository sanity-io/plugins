...
async function uploadAssetForRecovery(
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to download asset "${doc.originalFilename ?? doc._id}" from origin (${res.status} ${res.statusText})`,
    )
  }
  ...
  // Same single-doc merge as the main duplication path: keep original metadata
  // (altText/title/…) under the uploaded `_id` so SVG remaps don't leave an orphan.
  return {
    docs: [{...doc, _id: assetDoc._id, url: assetDoc.url, path: assetDoc.path}],
...
export default function Duplicator(props: DuplicatorProps) {
  ...
  async function handleDuplicate() {
    ...
    async function processDoc(doc: SanityDocument): Promise<void> {
      ...
      // `assetDownloadInit` only attaches the bearer token for Sanity CDN hosts.
      const res = await fetch(downloadUrl, assetDownloadInit(downloadUrl, token, typeIsFile))
