...
export type AssistDocumentContextValue = (
   ...
   * (a virtual draft) while this is false because the draft sibling is missing.
   * The same applies to a published variant with no draft sibling yet.
   */
  documentIsAssistable: boolean
  /**
   * True while the document store still has an in-flight commit (for example
   * the mutation that materializes a draft from published values, or a creatable
   * draft variant).
   */
