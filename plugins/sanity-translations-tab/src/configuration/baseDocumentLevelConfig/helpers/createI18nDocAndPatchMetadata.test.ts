...
test('replaces an existing entry in language-field metadata, preserving its _key', async () => {
...
test('replaces an existing entry in legacy metadata, mirroring the legacy format', async () => {
  const {client, insert} = createMockClient()
  const translationMetadata: SanityDocumentLike = {
    ...metadataBase,
    translations: [
...
test('appends a new entry in the language-field format with a random _key', async () => {
  const {client, insert} = createMockClient()
  const translationMetadata: SanityDocumentLike = {
    ...metadataBase,
  ...
  expect(insert).toHaveBeenCalledWith('after', 'translations[-1]', [
    {
      //a random key, not the locale id
      _key: expect.not.stringMatching(/^de$/),
      _type: 'internationalizedArrayReferenceValue',
      language: 'de',
      value: expectedValue,
    },
